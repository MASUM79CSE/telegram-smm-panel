  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 text-sm text-slate-400">
            <span>{(ticket.userId as unknown as { email?: string })?.email}</span>
            <StatusBadge status={ticket.status} />
            <span>{ticket.priority} priority</span>
          </div>
        </div>
        {ticket.status !== "CLOSED" && <CloseTicketButton ticketId={ticket._id.toString()} />}
      </div>
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <p className="text-sm font-medium text-white">
                {message.isAdmin ? "Support Team" : (message.senderId as unknown as { name?: string })?.name || "User"}
              </p>
      ...
      {ticket.status !== "CLOSED" && (
        <div className="mt-6">
          <TicketReplyForm ticketId={ticket._id.toString()} adminPath />
        </div>
      )}
    </div>
  );
