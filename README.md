...
- **Wallet ledger correctness** — MongoDB multi-document transactions + optimistic concurrency (`Wallet.version`) so concurrent orders can never double-spend or leave a wallet negative
...
| Database | MongoDB Atlas via [Mongoose](https://mongoosejs.com) — replica set required for transactions |
| Auth | [Auth.js / NextAuth v5](https://authjs.dev) — Credentials provider, JWT sessions |
| Telegram bot | [grammY](https://grammy.dev) + `@grammyjs/conversations` + `@grammyjs/storage-mongodb` |
...
└───────────────────────┬──────────────────────────────────────────┘
                        ▼
             MongoDB Atlas (replica set — required for
             multi-document transactions on wallet/order logic)

Background workers (separate long-running processes, or
serverless cron endpoints on Vercel — see docs/ARCHITECTURE.md §6):
  process-orders.ts             — dispatches PENDING orders to providers
  poll-order-status.ts          — reconciles IN_PROGRESS orders, issues partial refunds
  compute-delivery-estimates.ts — refreshes each service's median ETA
```
...
lib/            Core logic: db connection, auth config, env validation,
                business services (lib/services/*), Telegram bot (lib/telegram/*)
models/         Mongoose schemas (one file per collection) + registry
scripts/        Seed, background workers, Telegram webhook setup, local dev DB
...
- **Node.js 20+**
- A **MongoDB Atlas** cluster (free M0 tier works) — or an ephemeral local replica set, no Atlas account needed (see below)
...
The app requires a MongoDB **replica set** (wallet/order logic uses multi-document ACID transactions). Atlas clusters — including the free M0 tier — are always replica sets, so this is automatic once you point `MONGODB_URI` at one.

No Atlas account yet? Spin up an ephemeral in-memory replica set instead:

```bash
npx tsx scripts/start-dev-db.ts
```

This prints a `MONGODB_URI` to paste into `.env.local`. Data does **not** persist across restarts — development/testing only.
...
Open [http://localhost:3000](http://localhost:3000). Then seed an initial admin account:
...
|---|---|---|
| `MONGODB_URI` | **Required** | MongoDB connection string (Atlas or local dev replica set). |
| `AUTH_SECRET` | **Required** | Auth.js session/JWT signing secret. `openssl rand -base64 32` to generate one. |
...
| `CRON_SECRET` | Required to use `/api/cron/*` | Authenticates the serverless background-job endpoints (see [Deployment](#-deployment)). |
...
|---|---|---|
| **Unit** | `npm run test` | 201 tests — money math, validation, auth logic, rate limiting, all 44 API route auth/authorization boundaries |
| **Integration** | `npm run test:integration` | 35 tests against a real, ephemeral MongoDB **replica set** — exercises the actual wallet-transaction and concurrency-safety code paths, including a genuine race-condition test |
| **End-to-end** | `npm run test:e2e` | Playwright, real browser against a real ephemeral DB |

See [`docs/PRODUCTION_READINESS.md` §15](docs/PRODUCTION_READINESS.md#15-automated-testing) for full detail on what each suite covers and what's intentionally out of scope.
...
2. **Set environment variables** on the Vercel project (see [above](#-environment-variables) — at minimum `MONGODB_URI`, `AUTH_SECRET`, `AUTH_TRUST_HOST=true`, `NEXT_PUBLIC_APP_URL`, `ALLOWED_ORIGINS`, `CRON_SECRET`).
...
4. **Set GitHub Actions repository secrets** so the scheduler and health checks can reach your deployment:
   - `CRON_SECRET`, `PROD_URL` — drives `.github/workflows/cron.yml`, which calls `GET /api/cron/process-orders`, `/poll-order-status`, and `/compute-delivery-estimates` on a schedule.
...
Not on Vercel? Any platform that can run long-lived Node processes works too — run the three worker scripts (`process-orders`, `poll-order-status`, `compute-delivery-estimates`) with `-- --loop` under systemd/pm2/Docker instead of using the cron endpoints.
...
|---|---|
| App crashes on startup: "Invalid environment configuration" | A required env var is missing/invalid — check against `.env.example`. |
| `UntrustedHost` error on every login (fine locally, breaks after deploy) | Set `AUTH_TRUST_HOST=true` in production — Auth.js only auto-trusts the `Host` header outside `NODE_ENV=production`. |
...
| Local DB needs a replica set, plain `mongod` fails | Use `scripts/start-dev-db.ts` or Atlas — transactions require a replica set. |
...
| [`docs/DATABASE.md`](docs/DATABASE.md) | MongoDB schema, relationships, indexes, transactions, data lifecycle, backup/recovery |
...
Built with Next.js, TypeScript, and MongoDB · [Report an issue](https://github.com/MASUM79CSE/telegram-smm-panel/issues)
