...
```
 ...
 Browser  ─HTTPS──▶  │        Next.js server          │
                     │  (pages + /api/* route handlers)│ ──▶ MongoDB Atlas (replica set)
 Telegram ─Webhook─▶ │  (also mounts the bot at        │
   servers            │   /api/telegram/webhook)        │
                     └───────────────────────────────┘
                                    ▲
                                    │ shares the same Mongoose
                                    │ connection object (module cache)
                     ┌───────────────────────────────┐
                     │ scripts/process-orders.ts       │ ──▶ MongoDB Atlas
                     │ (separate OS process, --loop)   │      (same cluster)
                     └───────────────────────────────┘
```
...
lib/services/*          Business/domain logic layer — the ONLY place that
  ...
  └── calls ──▶
models/*.ts             Data access layer — Mongoose schemas/models, one file
                        per MongoDB collection. No business rules here beyond
                        schema-level validation/defaults.
  └── uses ──▶
lib/db.ts               Connection layer — cached Mongoose connection,
                        registers all models via models/index.ts side effect.
```
...
- `lib/money.ts` — fixed-point (`Decimal128`) arithmetic helpers for all monetary math; deliberately avoids floating point.
...
**Why a `lib/services` layer instead of putting logic directly in route handlers:** it is the single place where the Telegram bot and the website converge, so the same MongoDB transaction/validation runs no matter which surface a user acted from. It also keeps route handlers as thin HTTP-protocol adapters (auth check → rate limit → parse input → call service → map result/errors to a response).
...
- **Split config rationale:** `auth.config.ts` contains only Edge-safe logic (no bcrypt/Mongoose imports) so `proxy.ts` can run without pulling in Node-only APIs; `auth.ts` extends it with the actual `Credentials` provider (which does need bcrypt + Mongoose) and is only imported by server-side code (route handlers, server components) — never by `proxy.ts`.
...
placeOrder():
  ...
  3. Compute charge = rate * quantity / 1000 (lib/money.ts, Decimal128-safe)
  4. mongoose.startSession() → withTransaction():
       a. Load Wallet, check balance >= charge
       b. Optimistic-concurrency update: Wallet.updateOne({_id, version: N}, {$set balance, $inc version})
          — if modifiedCount !== 1, another concurrent write already changed
...
There is no cron-based scheduler beyond these three workers; `VerificationToken` documents self-expire via a MongoDB TTL index (`expireAfterSeconds: 0` on `expiresAt`) rather than a cleanup job — see [`DATABASE.md` §7](DATABASE.md#7-data-lifecycle-retention-and-ttl-indexes).

## 7. Telegram Bot Integration Detail

- **Library:** grammY, with `@grammyjs/conversations` for multi-step flows (e.g. `/order` walks category → service → target → quantity) and `@grammyjs/storage-mongodb` for session persistence (stored in a `telegram_bot_sessions` collection on the same MongoDB connection — no second database).
- **Two run modes:**
...
- **Transactional safety:** any error thrown inside a `mongoSession.withTransaction()` callback aborts the entire transaction — partial state changes (e.g. a wallet debit without a matching order) cannot occur.
...
- **Stateless web tier:** the Next.js app holds no meaningful in-process state other than the cached Mongoose connection and (fallback-only) in-memory rate-limit counters — it can run as multiple horizontally-scaled instances, **except** that the in-memory rate-limit fallback and long-polling bot mode do not work correctly across multiple instances (use Upstash Redis + webhook mode in any multi-instance deployment).
- **Connection pooling:** `lib/db.ts` caches the Mongoose connection across serverless invocations/hot reloads (`global.__mongooseCache`) with `maxPoolSize: 10`, avoiding Atlas connection-limit exhaustion under bursty serverless traffic.
...
- **Defense in depth on money movement:** every balance change requires (a) an active MongoDB transaction, (b) an optimistic-concurrency version check on the `Wallet` document, and (c) an immutable `Transaction` ledger row — so double-spends and lost updates under concurrent requests are structurally prevented, not just discouraged by convention.
- **Secrets at rest:** provider API keys are AES-256-GCM encrypted (`lib/crypto.ts`) using a key derived from `AUTH_SECRET`; they are also excluded from default Mongoose query projections (`select: false` on the schema field) so a stray `Provider.find()` never accidentally returns them.
- **Tokens:** email verification, password reset, and Telegram-link codes are never stored raw — only their SHA-256 hash (`VerificationToken.tokenHash`) — and expire via a MongoDB TTL index.
- **Idempotency:** deposit approval and order refund both write a deterministic `idempotencyKey` (e.g. `payment-approval:<id>`) on their `Transaction` row and use an atomic "claim" update (`updateOne` with a status guard) before doing any money movement, preventing double-processing from a retried request or a double-click.
...
| Decision | Rationale |
|---|---|
| MongoDB + Mongoose over the originally-planned PostgreSQL + Prisma | User-directed change; Mongoose's flexible schema suits the evolving service/provider catalog, and MongoDB Atlas's free tier meets the project's budget constraints. |
...
| Decimal128 + custom fixed-point helpers (`lib/money.ts`) instead of floats or a third-party decimal library | Avoids floating-point rounding bugs in money math without adding a dependency; explicitly called out in code comments as a lesson carried over from an earlier Prisma/`Decimal` design. |
